...
public class MailSender{
	...
	public void composeMessage(String recipient,  HttpServletRequest request) throws ApplicationException {
		LOG.debug("MailSender started");
		Properties properties = loadMailProperties(request);
		Session session = createSession(properties);
		prepareAndSend(session, recipient);
	...
	private void prepareAndSend(Session session, String recipient) throws ApplicationException {
		LOG.debug("Preparing message");
		Message message = new MimeMessage(session);
		String link = "<a href = \"http://localhost:8080/finalTask/controller?command=verify&email="+recipient+"&code="+"\">this link</a>";
